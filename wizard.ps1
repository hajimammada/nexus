...
try {
    ...
    if (-not (Test-Path $xamlFile)) {
        [System.Windows.Forms.MessageBox]::Show("Could not find wizard.xaml in: $scriptDir`nPlease ensure all files are extracted from the ZIP.", "Nexus Setup Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        exit 1
    }
    ...
    [System.Windows.Forms.MessageBox]::Show($_.Exception.ToString(), "Nexus Setup Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
}
